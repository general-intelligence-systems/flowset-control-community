...
@FragmentDescriptor("batch-id-column-fragment.xml")
@RendererItemContainer("batchDc")
public class BatchIdColumnFragment extends EntityDetailLinkFragment<HorizontalLayout, BatchData> {
    @ViewComponent
    protected JmixButton idBtn;

    @Override
    public void setItem(@NonNull BatchData item) {
        super.setItem(item);
    ...
    public void onIdBtnClick(final ClickEvent<JmixButton> event) {
        openDetailView(BatchData.class);
    }
}
