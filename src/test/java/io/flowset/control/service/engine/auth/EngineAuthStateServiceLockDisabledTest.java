...
public class EngineAuthStateServiceLockDisabledTest extends AbstractIntegrationTest {
    ...
    @Test
    @DisplayName("Engine is not locked when lock support is disabled")
    void testGivenExistingLockedState_whenIsLocked_thenFalse() {
        //given
    ...
    @Test
    @DisplayName("No engine auth state found when lock support is disabled")
    void testGivenExistingAuthState_whenFindByEngineId_thenNull() {
        //given
        BpmEngine engine = createEngine();
    ...
    @Test
    @DisplayName("Engine auth state is not created when lock support is disabled")
    void testGivenNoAuthState_whenRegisterAccessTokenFailure_thenStateNotCreated() {
        //given
        BpmEngine engine = createEngine();
    ...
    @Test
    @DisplayName("Engine auth state is not removed when lock support is disabled")
    void testGivenExistingAuthState_whenUnlock_thenStateNotRemoved() {
        //given
        BpmEngine engine = createEngine();
        createEngineAuthState(engine, 2, true);
    ...
    BpmEngine createEngine() {
        ...
        engine.setName("auth-state-disabled-" + UUID.randomUUID().toString().substring(0, 8));
        engine.setType(EngineType.CAMUNDA_7);
        engine.setBaseUrl("http://localhost:28080/engine-rest/" + UUID.randomUUID());
